...
  if (token == null) return res.sendStatus(401); // Si no hay token, acceso denegado

  jwt.verify(token, process.env.JWT_SECRET || 'your-secret-key', (err, user) => {
    if (err) return res.sendStatus(403); // Token inválido o expirado
    req.user = user; // Guarda el usuario decodificado en el request
...
// AHORA PROTEGE LAS RUTAS CON verifyToken
app.get('/api/users/by-email', verifyToken, async (req, res) => {
  try {
...
// AHORA PROTEGE LA RUTA DE USUARIOS
